...
contract CrocssPluginSetup is PluginSetup {
    using Address for address;
    using Clones for address;
    using ERC165Checker for address;

    /// @notice The address of the `TokenVoting` base contract.
    CrocssPlugin private immutable tokenVotingBase;
    ...
    /// @param addr The token address. If this is `address(0)`, a new `GovernanceERC20` token is deployed. If not, the
    /// existing token is wrapped as an `GovernanceWrappedERC20`.
    ...
    /// @notice The contract constructor deploying the plugin implementation contract and receiving the governance token
    /// base contracts to clone from.
    ...
    constructor(GovernanceERC20 _governanceERC20Base, GovernanceWrappedERC20 _governanceWrappedERC20Base) {
        tokenVotingBase = new CrocssPlugin();
        ...
        returns (address plugin, PreparedSetupData memory preparedSetupData)
    {
...

        {
            ...
            plugin = createERC1967Proxy(address(tokenVotingBase), encodedData);
        }
      ...
      tokenSettings.addr != address(0) ? 3 : 4
        ...
        if (tokenSettings.addr == address(0)) {
            ...
            permissions[3] = PermissionLib.MultiTargetPermission(
                PermissionLib.Operation.Grant, token, _dao, PermissionLib.NO_CONDITION, tokenMintPermission
            );
        }
